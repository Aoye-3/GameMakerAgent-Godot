...
1. Require an existing native Godot project. Run `gamemaker doctor --project <path> --live`; distinguish file readiness, live connection and recorded conformance. An untested provider needs explicit conformance, not simulated success. Do not install third-party tools unless authorized.
...
5. Send only the Asset Spec to the selected mature asset provider. Archive its original in project artifacts, then use `gamemaker asset normalize --project <path> --spec <spec.json> --input <original>` and record the returned item inside a `normalized-assets` object with `schema_version` and `assets`. Reject failed transparency/content checks before Godot receives it; never silently substitute a placeholder.
...
Use `gamemaker record --project <path> --work <id> --kind <kind> --input <json>` for schema-validated records and the rebuildable Dock index. Decision Card JSON is authoritative; Markdown is derived. Evidence must name the implementation ID, content revision, provider/version, session/run and artifact SHA-256 values. After implementation, re-query the native project for its current fingerprint. Finish with `gamemaker evidence review --project <path> --work <id>`; file-only review cannot certify current code.
