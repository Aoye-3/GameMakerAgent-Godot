...
用户已于 2026-09-04 明确允许内置生图暂存到 Codex 的 C 盘生成目录；使用前把原图归档到
项目 artifacts 并记录哈希。该例外不适用于项目代码、依赖或测试缓存。

## 执行清单与门禁

1. **准备与真实连接（已实测）。** 用户在原生 Project Manager 创建
...
2. **Core 修复（本轮范围已通过回归）。** 真实输入映射和 UID 入口；排除第三方源码干扰；Decision JSON
...
- 已通过真实 MCP 完成 Project1 场景、脚本、Resource、输入、素材导入、运行、状态、截图、
  日志和停止；200 → 240 增量修改、重复节点/输入检查及编辑器重连通过。
- 最新证据 `collect-trial-r1203103-3` 对应当前内容指纹；脚本改动导致旧证据拒绝，恢复后重新
  运行得到 PASS。Dock 在真实项目中的加载刷新哈希对比通过。
- 本轮部分 Production/Decision 记录是执行后补录，未宣称已经完成一轮独立的
  “全新自然语言 → Skill 自动路由 → 制品先行”用户验收；这是下一次手动验收重点。
- 详细结果与限制见 [真实试跑报告](manual-trial-verification-2026-09-04.md)。不晋升、不发布 0.1.0。
